...
function spinControl (el, args, options){
	...
	this.thumb.addEventListener(window.Touch ? 'touchstart' : 'mousedown', this, true);
...
spinControl.prototype = {
	...
	destroy: function(){
		this.destroyLabels();
		if (this.thumb){
			this.thumb.removeEventListener(window.Touch ? 'touchstart' : 'mousedown', this, true);
	...
	onTouchStart: function(e) {
		if(window.Touch && e.targetTouches.length != this.numberOfTouches )
			return;
		...
		this.startX = window.Touch ? e.targetTouches[0].clientX : e.clientX;
		this.startY = window.Touch ? e.targetTouches[0].clientY : e.clientY;
		this.startTime = e.timeStamp;
		
		this.thumb.addEventListener(window.Touch ? 'touchmove' : 'mousemove', this, false);
		this.thumb.addEventListener(window.Touch ? 'touchend' : 'mouseup', this, false);
	...
	onTouchMove: function(e) {
		...
		var x =  window.Touch ? e.targetTouches[0].clientX : e.clientX;
		var y =  window.Touch ? e.targetTouches[0].clientY : e.clientY;
	...
	onTouchEnd: function(e) {
		e.preventDefault(); // no preventDefault to allow clicks on thumb
		this.thumb.removeEventListener(window.Touch ? 'touchmove' : 'mousemove', this, false);
		this.thumb.removeEventListener(window.Touch ? 'touchend' : 'mouseup', this, false);
